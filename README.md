...
M7B fixes one reproducible 4-file / 60,967-byte development snapshot generated from public source
`8a4016a9c154238cd7e5df5d1a3ed8fd194dd10d`. Start with the
[data card](datasets/sft-development-v1/DATA_CARD.md) for provenance, fields, hashes, and limits,
then open the [generated candidate report](datasets/sft-development-v1/candidate/report.md).
The report's `not a tracked dataset` sentence is an M7A generation-time boundary.
M7B versions the same generated bytes for review; they remain not training-ready and retain the
`development_candidate` stage.
The six-sample [review packet](datasets/sft-development-v1/HUMAN_REVIEW.md) records that guided human review is complete: 6/6 pre-registered samples passed. This covers 6/18 candidate samples; the remaining 12 were not individually reviewed, and the candidate remains not training-ready.
...
leakage contract; they are not a training run, a training-ready claim, or evidence that a model
improved. [ADR 0013](docs/decisions/0013-m7a-sft-data-contract.md) records the export boundary, and
